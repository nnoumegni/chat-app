...
import {DmRoom, Room, RoomUser} from "../../models/chat-models";
...
export const AsideLeft = () => {
    ...
    const handleDmMessaging = (dmUser: RoomUser = undefined) => {
        ...
        const roomUri = Utils.getDmRoomUri({dmUser, currentUser: user});
        const fromUser: RoomUser = {userId: user.id, fullname: user.fullname};
        const room: Room = {name: dmUser.fullname, addedBy: user.id, users: [fromUser, dmUser], roomUri, uri: roomUri, type: 'dm', active: 0};
    ...
    return (
        <div className="tyn-aside tyn-aside-base">
            ...
            <div className="tyn-aside-body flex flex-col overflow-hidden">
                {!showNewRoomForm && (
                    <>
                        ...
                        <div className="tab-content overflow-auto">
                            <div className="tab-pane show active" id="all-chats" tabIndex={0} role="tabpanel">
                                <ul className="tyn-aside-list">
                                    {rooms.map((room, idx) => {
                                        const {name, type, uri, roomUri} = room;
                                        const selectedRoomUri = selectedRoom?.uri || selectedRoom?.roomUri;
                                        const currentRoomUri = uri || roomUri;
                                        const activeCls = selectedRoomUri && currentRoomUri && selectedRoomUri === currentRoomUri ? 'active' : '';

                                        return (
                                            <li key={idx} className={'tyn-aside-item js-toggle-main ' + activeCls} onClick={(evt: MouseEvent) => handleRoomClick({room})}>
                                                    <div className="tyn-media-group">
                                                        <div className="tyn-media tyn-size-lg border border-">
                                                            {type === 'dm' && (
                                                        ...
                                                        <div className="tyn-media-col">
                                                            <div className="tyn-media-row">
                                                                <h6 className="name capitalize">{name}</h6>
                ...
                {showNewRoomForm && (
                    <NewRoomForm handleRoomClick={handleRoomClick}/>
                )}
            </div>
        </div>
    )
